...
$NAME_OBJECT_CONNECTION = $fieldsFill['NAME_OBJECT_CONNECTION'] == 'Иное'? 'Иное: '.$fieldsFill['ANOTHER'] : $fieldsFill['NAME_OBJECT_CONNECTION'];

$document->setValue('r6', $NAME_OBJECT_CONNECTION);
...
if($userType !== 'Физическое лицо' and $userType != 'Юридическое лицо') {
    ...
    if(!$userInfo['UF_HEAD_SURNAME']) {
        $document->setValue('userType', 'Индивидуальный предприниматель ' . $userInfo['LAST_NAME'] . ' ' . $userInfo['NAME'] . ' ' . $userInfo['SECOND_NAME']);
    } else {
        $document->setValue('userType', 'Индивидуальный предприниматель ' . $userInfo['UF_HEAD_SURNAME'] . ' ' . $userInfo['UF_HEAD_NAME'] . ' ' . $userInfo['UF_HEAD_MIDDLE']);
    }
} else {
...
foreach ($fieldsFill['MAX_P_USHE_P'] as $k=>$toch) {
    $table->addRow();
    // if($fieldsFill['POWER_B']) {
        //     $table->addCell(1000, $styleCell)->addText('Точка '.($k+1).' ', $fonts);
        //     $table->addCell(250, [])->addText($toch.' кВт', $fonts);
        //     $table->addCell(1000, $styleCell)->addText($fieldsFill['POWER_B'].' кВ', $fonts);
        // } else {
        $ranee = ($fieldsFill['MAX_P_RANEE_P'][$k]) ? $fieldsFill['MAX_P_RANEE_P'][$k] : 0;
        $table->addCell(1000, $styleCell)->addText('Точка '.($k+1).' '.($toch + $ranee).' кВт', $fonts);
    // }
}

$document->setComplexBlock('poinCp', $table);

$document->setValue('sPo', $fieldsFill['MAX_P_RANEE_P'] ? array_sum((array)($fieldsFill['MAX_P_RANEE_P']??[])) : '');
...
$document->setValue('maxPc', $fieldsFill['MAX_P_USHE_P'] ? array_sum((array)($fieldsFill['MAX_P_USHE_P']??[])) : '-');
$document->setValue('maxPcB', $fieldsFill['MAX_POWER_CONNECT'] ? $fieldsFill['POWER_B'].' кВ' : '-');


$document->setValue('maxPcBe', array_sum((array)($fieldsFill['MAX_P_RANEE_P']??[])));
